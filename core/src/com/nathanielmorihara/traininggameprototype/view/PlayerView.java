...
//    private static Animation<TextureRegion> walkRightAnimation;
    ...
    public static void load() {
        ...
        walkSheet = new Texture(Gdx.files.internal("MyFirstGuy.png"));
...
//        walkRightAnimation = new Animation<TextureRegion>(0.1f, walkRight);
    ...
    public void draw(SpriteBatch spriteBatch, Camera camera, Viewport viewport, float time, PlayerModel playerModel) {
...
//        }
        ...
        Vector2 playerVec = new Vector2(playerModel.body.getPosition());

        // Okay so this is necessary in order to translate the player coordinates in the world to where the camera is
        // positioned...otherwise, the  x,y given would be relative to the screen, not the world
        spriteBatch.setProjectionMatrix(camera.combined);

        spriteBatch.draw(
                currentFrame,
                playerVec.x - playerModel.width/2,
                playerVec.y - playerModel.height/2,
                playerModel.width,
                playerModel.height);
    }
}
